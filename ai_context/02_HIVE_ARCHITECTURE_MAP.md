...
- `ContentTypeRoutingStage`: early deterministic routes (videos, strong screenshots, receipts/docs, architecture/places, etc).
- `WeightedCategoryScoringStage`: base scoring + per-cell boosts.
- `VetoPrecedenceStage`: deterministic precedence and veto rules (memes vs people, books precedence, etc).
- `CategoryEntryGateStage`: per-category entry gates that can veto weak/unsafe placements.
